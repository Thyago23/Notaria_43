// ============================================
// Seed automático al arrancar el servidor
// Se ejecuta solo si la base de datos está vacía.
// Usa queries SQL directas (no Prisma) para ser
// compatible con la estructura creada por init.sql.
// ============================================

import { getDatabase } from './database.js';
import { hashValue } from '../modules/wrappers/hash.wrapper.js';
 ...
 * Se invoca al arrancar el servidor.
 */
export async function runSeedIfEmpty() {
  ...
  try {
    ...
    if (userCount === 0) {
      console.log('🌱 Base de datos vacía, creando usuarios iniciales...\n');

      for (const userData of SEED_USERS) {
        const hashedPassword = await hashValue(userData.plainPassword);
        ...
        console.log(`  ✅ ${userData.role}: ${userData.nombres} ${userData.apellidos} (${userData.cedula})`);
      }
      ...
      console.log('📦 Usuarios existentes detectados, omitiendo creación de usuarios.');
    }

    console.log('🌱 Asegurando que los trámites del seed estén presentes en la base de datos...\n');

    for (const tramiteData of SEED_TRAMITES) {
      const tramite = await database.tramite.upsert({
        where: { nombre: tramiteData.nombre },
        update: {},
        create: tramiteData,
      });
      console.log(`  ✅ Trámite asegurado: ${tramite.nombre} (${tramite.duracionMinutos} min)`);
    }

    console.log('\n🎉 Seed completado exitosamente!');
    console.log('');
  } catch (error) {
    // El seed no es crítico — si falla, el servidor sigue funcionando
    console.warn('⚠️  Error en seed automático (no crítico):', error.message);
  }
}
